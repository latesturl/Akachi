...
const handler = async (m, { conn, args, text }) => {
  ...
  try {
    ...
    // Check if there's a number in the text
      ...
      return m.reply("Tag users to promote, reply to their message, or enter their number")
    }

    await conn.groupParticipantsUpdate(m.key.remoteJid, users, "promote")
    m.reply(`Successfully promoted ${users.map((v) => "@" + v.split("@")[0]).join(", ")}`, null, { mentions: users })
  } catch (error) {
    console.error("Promote error:", error)
    m.reply(`Failed to promote: ${error.message}`)
  }
}

handler.help = ["promote @user/reply"]
handler.tags = ["group"]
